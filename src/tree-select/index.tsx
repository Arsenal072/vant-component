...
function TreeSelect(
  ...
  function Content() {
    ...
    return subItems.map(item => (
      ...
      >
        <div class={bem('text')}>{item.text}</div>

        <div class={bem('add-wrap')}>
          {item.tag && (
            <Tag round plain type={item.tag.type || 'default'}>
              {item.tag.name}
            </Tag>
          )}
          {item.add && <div class={bem('add')}>{item.add}</div>}
          {arrow && <Icon name="arrow" size="16px" class={bem('selected')} />}
        </div>
      </div>
    ));
  }
  ...
  return (
    <div
      class={bem({ border: reverseBorder })}
    ...
    >
      ...
      <div class={bem('content', { 'active-color': activeColor })}>
        {Content()}
      </div>
    </div>
  );
