...
export default createComponent({
  ...
  computed: {
    ...
    size() {
      return this.card
        ? this.computedWidth
        : this[this.vertical ? 'computedHeight' : 'computedWidth'];
    },

    trackSize() {
      return this.card
        ? this.size > 375
          ? this.size * 0.835 * this.count
          : this.size * 0.83 * this.count
  ...
  methods: {
    ...
    getTargetOffset(targetActive, offset) {
      ...
      if (this.card) {
        targetOffset +=
          this.size > 375 ? currentPosition * 0.168 : currentPosition * 0.173;
      }
  ...
  render() {
    return (
      <div class={bem([this.type])}>
        ...
        {this.genIndicator()}
        {this.type === 'mask' && <div class={bem('mask')}></div>}
      </div>
    );
  }
});
